...
For GLM_NSA SG prefill with eight heads and FP8 QK, the launcher in
`csrc/sparse_mla_sm120_prefill.cu` enables swapped QK operands by default.
`FLASHINFER_GLM_H8_PREFILL_SWAP_QK=0` restores the previous H8 QK implementation
for A/B comparisons; both variants retain padded-head pruning and packed PV
precision passes. Recreate cached CUDA graphs when changing this setting.
