...
void BatchMLAPagedAttentionRun(TensorView float_workspace_buffer, TensorView int_workspace_buffer,
  ...
  DISPATCH_context(
      DTypeQ, DTypeKV, DTypeO, IdType, MASK_MODE, HEAD_DIM_CKV, HEAD_DIM_KPE, Params, [&] {
        ...
        params.maybe_custom_mask = maybe_custom_mask.has_value()
                                       ? static_cast<uint8_t*>(maybe_custom_mask.value().data_ptr())
                                       : nullptr;
        params.maybe_mask_indptr = maybe_mask_indptr.has_value()
                                       ? static_cast<IdType*>(maybe_mask_indptr.value().data_ptr())
                                       : nullptr;
        params.maybe_mask_kv_len = maybe_mask_kv_len.has_value()
