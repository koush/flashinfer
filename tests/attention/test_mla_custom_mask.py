...
def test_mla_packed_custom_mask_ragged_byte_offsets():
    """Each request starts on a byte boundary, including non-byte-sized masks."""
    device = torch.device("cuda:0")
    torch.manual_seed(7)
    q = torch.randn(5, 16, CKV_DIM, dtype=torch.bfloat16, device=device)
    rope = torch.randn(5, 16, KPE_DIM, dtype=torch.bfloat16, device=device)
    kv = torch.randn(12, 1, CKV_DIM, dtype=torch.bfloat16, device=device)
    kv_rope = torch.randn(12, 1, KPE_DIM, dtype=torch.bfloat16, device=device)
    masks = [
        torch.rand(2, 5, device=device) > 0.5,
        torch.rand(3, 7, device=device) > 0.5,
    ]
    for mask in masks:
        mask[:, 0] = True
    mask = torch.cat([m.flatten() for m in masks])
    packed, _ = segment_packbits(
        mask,
        torch.tensor([0, 10, 31], dtype=torch.int32, device=device),
        bitorder="little",
    )
    wrapper = BatchMLAPagedAttentionWrapper(
        torch.empty(128 << 20, dtype=torch.uint8, device=device), backend="fa2"
    )
    wrapper.plan(
        qo_indptr=torch.tensor([0, 2, 5], dtype=torch.int32, device=device),
        kv_indptr=torch.tensor([0, 5, 12], dtype=torch.int32, device=device),
        kv_indices=torch.arange(12, dtype=torch.int32, device=device),
        kv_len_arr=torch.tensor([5, 7], dtype=torch.int32, device=device),
        num_heads=16,
        head_dim_ckv=CKV_DIM,
        head_dim_kpe=KPE_DIM,
        page_size=1,
        causal=False,
        sm_scale=1 / math.sqrt(CKV_DIM + KPE_DIM),
        q_data_type=q.dtype,
        kv_data_type=kv.dtype,
    )
    expected = wrapper.run(query=(q, rope), kv_cache=(kv, kv_rope), custom_mask=mask)
    actual = wrapper.run(
        query=(q, rope), kv_cache=(kv, kv_rope), packed_custom_mask=packed
    )
    torch.testing.assert_close(actual, expected, rtol=0, atol=0)


def attention_ref(
    batch_size, q, k, v, causal, sm_scale, custom_mask=None, prefix_len=None
):
    ...
    if custom_mask is not None and prefix_len is not None:
        ...
        full_mask = torch.ones(
            batch_size, qo_len, kv_len, dtype=torch.bool, device=q.device
        )
        ...
        # 0..(kv_len - qo_len + j)
        causal_mask = torch.arange(kv_len - qo_len, kv_len, device=q.device).unsqueeze(
            1
...
def generate_kv_from_cache(ckv, kpe, kv_len, batch_size, num_heads):
    ...
    k = torch.cat(
        [
            k,
            kpe.view(batch_size, pages_num * page_size, KPE_DIM)[
                :, :kv_len, :
            ].contiguous(),
        ],
        dim=-1,
    ...
    k = (
        ...
        .reshape(batch_size * kv_len, num_heads, -1)
    )
...
def test_mla_custom_mask_causal_equivalent(
    batch_size, kv_len, qo_len, num_heads, page_size, dtype
):
    ...
    q_nope = torch.randn(
        batch_size * qo_len, num_heads, CKV_DIM, dtype=dtype, device=device
    )
    q_pe = torch.randn(
        batch_size * qo_len, num_heads, KPE_DIM, dtype=dtype, device=device
    )
    pages_num = math.ceil(kv_len / page_size)
    ckv = torch.randn(
        batch_size * pages_num, page_size, CKV_DIM, dtype=dtype, device=device
    )
    kpe = torch.randn(
        batch_size * pages_num, page_size, KPE_DIM, dtype=dtype, device=device
    )
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_custom_mask_all_ones_matches_noncausal(
    batch_size, kv_len, qo_len, num_heads, page_size, dtype
):
    ...
    all_ones_mask = torch.ones(
        batch_size, qo_len, kv_len, dtype=torch.bool, device=device
    )
    wrapper_custom = BatchMLAPagedAttentionWrapper(workspace_buffer, backend="fa2")
    wrapper_custom.plan(
        q_indptr,
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_custom_mask_sliding_window(
    batch_size, kv_len, qo_len, num_heads, page_size, dtype
):
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_no_mask_regression(
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_tree(
    batch_size, prefix_len, num_prefill_tokens, num_heads, page_size, dtype
):
    ...
    tree_mask = _build_tree_mask(
        num_prefill_tokens, prefix_len
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_all_ones_suffix(
    batch_size, prefix_len, num_prefill_tokens, num_heads, page_size, dtype
):
    ...
    suffix_mask = torch.ones(
        batch_size, qo_len, num_prefill_tokens, dtype=torch.bool, device=device
    )
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_vs_reference(
    batch_size, prefix_len, num_prefill_tokens, num_heads, page_size, dtype
):
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_causal_suffix(
    batch_size, prefix_len, num_prefill_tokens, num_heads, page_size, dtype
):
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_no_prefix(
    batch_size, qo_len, num_heads, page_size, dtype
):
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_extended_tree(
    batch_size,
    prefix_len,
    num_prefill_tokens,
    extra_kv_cache,
    ...
    tree_mask_prefill = _build_tree_mask(
        num_prefill_tokens, prefix_len
    ...
    packed_mask, mask_indptr_gpu = _pack_suffix_mask(
        suffix_mask, qo_len, mask_kv_len, batch_size, device
    )
    mask_kv_len_tensor = torch.full(
        (batch_size,), mask_kv_len, dtype=torch.int32, device=device
    )
    ...
    o_cc = wrapper_cc.run(
        ...
        ckv,
        kpe,
        causal_custom_mask=packed_mask,
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_extended_vs_reference(
    batch_size,
    prefix_len,
    num_prefill_tokens,
    extra_kv_cache,
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_extended_causal_equivalent(
    batch_size,
    prefix_len,
    num_prefill_tokens,
    extra_kv_cache,
...
@pytest.mark.parametrize("page_size", [1])
@pytest.mark.parametrize("dtype", [torch.float16])
def test_mla_causal_custom_mask_extended_all_ones(
    batch_size,
    prefix_len,
    num_prefill_tokens,
    extra_kv_cache,
    ...
    suffix_mask = torch.ones(
        batch_size, qo_len, mask_kv_len, dtype=torch.bool, device=device
    )
    packed_mask, mask_indptr_gpu = _pack_suffix_mask(
        suffix_mask, qo_len, mask_kv_len, batch_size, device
    )
