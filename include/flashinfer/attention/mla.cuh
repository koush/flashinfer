...
template <bool CAUSAL_, bool USE_CUSTOM_MASK_, bool USE_CAUSAL_CUSTOM_MASK_, uint32_t NUM_STAGES_,
          bool QK_SHARD_, uint32_t HEAD_DIM_CKV_, uint32_t HEAD_DIM_KPE_, uint32_t CTA_TILE_Q_,
...
__device__ __forceinline__ void logits_mask_(
    ...
    typename KTraits::DTypeQKAccum (*s_frag)[8], const uint32_t cp_world_size,
    const uint32_t cp_rank, const uint32_t causal_kv_len, const uint8_t* custom_mask_ptr,
    const uint32_t mask_kv_len, const uint32_t prefix_len) {
  const uint32_t lane_idx = threadIdx.x, warpgroup_idx = threadIdx.z, warp_idx_in_wg = threadIdx.y;
...
#pragma unroll
      for (uint32_t reg_id = 0; reg_id < 8; ++reg_id) {
        ...
        if constexpr (KTraits::USE_CUSTOM_MASK) {
          const uint64_t global_kv_idx =
              cp_world_size > 0 ? (static_cast<uint64_t>(kv_idx) * cp_world_size + cp_rank)
                                : static_cast<uint64_t>(kv_idx);
          ...
          if (global_kv_idx < prefix_len) {
            ...
            const uint64_t offset =
                static_cast<uint64_t>(q_idx) * mask_kv_len + (global_kv_idx - prefix_len);
            const uint32_t byte_offset = static_cast<uint32_t>(offset / 8);
            mask = (q_idx < qo_len && kv_idx < kv_end && byte_offset < mask_total_bytes)
                       ? ((custom_mask_ptr[byte_offset] >> (offset % 8)) & 1)
                       : false;
          ...
          const uint32_t causal_kv_idx =
              cp_world_size > 0 ? (kv_idx * cp_world_size + cp_rank) : kv_idx;
...
#pragma unroll 1
  for (IdType work_idx = work_indptr[blockIdx.y]; work_idx < work_indptr[blockIdx.y + 1];
    ...
    const uint32_t kv_len = params.cp_world_size
                                ? (params.kv_len[work_idx] / cp_world_size +
                                   (params.kv_len[work_idx] % cp_world_size > cp_rank))
                                : params.kv_len[work_idx];
    ...
    int kv_tile_idx =
        ceil_div((CAUSAL ? min(kv_end, causal_kv_len - q_len +
                                           (packed_qo_start + cluster_tile_q) / num_heads)
                         : kv_end),
                 CTA_TILE_KV) -
...
cudaError_t BatchMLAPagedAttention(Params params, uint32_t num_blks_x, uint32_t num_blks_y,
  ...
  DISPATCH_SMEM_CONFIG(smem_limit_per_sm, NUM_STAGES, CTA_TILE_KV, QK_SHARD, {
    ...
    using KTraits =
        KernelTraits<CAUSAL, USE_CUSTOM_MASK, USE_CAUSAL_CUSTOM_MASK, EFF_NUM_STAGES, EFF_QK_SHARD,
